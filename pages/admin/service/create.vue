...
const handleSubmitService = async () => {
    ...
    if(!perfil.userId){
        messageError.value.verify = "Token não atenticado ou vencido faça login novamente."
        clearErrors(messageError)
        return
    }
...
<template>
    <AdminMenu />
    <main class="w-screen h-[90vh] flex flex-col justify-between items-center bg-[#202020de]">
        <section class="w-[100%] h-[15vh] grid place-items-center bg-[#202020]">
            ...
            <h3 class="text-xl font-bodoni font-bold tracking-wide text-[#c6a765] capitalize">
        ...
        <form class="w-[90%] h-[80vh] flex flex-col items-center justify-center" @submit.prevent="handleSubmitService">
            <InputLabel input_name="serviço" link="service" place="digite o nome do serviço." v-model="service" />
            <span v-show="messageError.service">{{
                messageError.service
                }}</span>
            <InputLabel input_name="Valor" link="price" place="digte o valor do serviço" v-model="price" />
            <span v-show="messageError.price">{{ messageError.price }}</span>
            <signupBtn btn-type="submit" btn-name="Cadastrar" :is-submitting="isSubmitting" />
        </form>
        <MsgError :mensagem="messageError.request" :state="!!messageError.request" />
